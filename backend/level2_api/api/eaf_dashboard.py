...
@api_view(["GET"])
def eaf_dashboard(request: Request) -> Response:
    """Return the read-only live operating picture for EAF-01."""
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            FROM equipment
            WHERE area = 'EAF' AND is_active = TRUE
            ORDER BY code
            LIMIT 1
            """
        )
        ...
        cursor.execute(
            ...
            WHERE h.status IN ('CHARGING', 'EAF', 'TAPPING')
        ...
        cursor.execute(
            ...
            WHERE pt.is_active = TRUE
              AND (e.area = 'EAF' OR lpv.tag_name LIKE 'EAF.%')
        ...
        cursor.execute(
            ...
            WHERE he.area = 'EAF' OR e.area = 'EAF'
