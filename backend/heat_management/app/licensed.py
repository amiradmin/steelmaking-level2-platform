...
def get_license_status(now: datetime | None = None) -> dict[str, Any]:
    ...
    if not token:
        ...
        return _base_state(
            status="READ_ONLY",
            read_only=True,
            message="No valid license is installed. The service is read-only.",
        )
    ...
    try:
        ...
        return _base_state(
            status="INVALID",
            read_only=True,
            message="The installed license is invalid. The service is read-only.",
        )


@app.get("/license", tags=["system"])
def license_status() -> dict[str, Any]:
    return get_license_status()


@app.middleware("http")
async def signed_license_guard(request: Request, call_next):
    if request.method.upper() in SAFE_METHODS or request.url.path in {"/health", "/license"}:
