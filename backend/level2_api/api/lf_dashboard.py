...
@api_view(["GET"])
def lf_dashboard(request: Request) -> Response:
    """Return the read-only live operating picture for LF-01."""
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            FROM equipment
            WHERE area = 'LF' AND is_active = TRUE
            ORDER BY code
            LIMIT 1
            """
        )
        ...
        cursor.execute(
            ...
            WHERE pt.is_active = TRUE
              AND (e.area = 'LF' OR lpv.tag_name LIKE 'LF.%')
